...
def signal_handler(sig, frame):
    ...
    global server_process_global # Make it accessible
...
def main():
    ...
    global server_process_global # Declare intent to modify global var
    ...
    client_parser.add_argument("--no-gpu", action="store_true", help="Disable GPU acceleration")
    client_parser.add_argument("--gpu-layers", type=int, default=0, help="Number of GPU layers to use (0=auto)")
    client_parser.add_argument("--quantize", choices=["4bit", "8bit"], help="Quantize the model to reduce memory usage")
    client_parser.add_argument("--prompt", help="Input text prompt")
    client_parser.add_argument("--temperature", help="Temperature for text generation")
    client_parser.add_argument("--max-tokens", help="Maximum number of tokens to be generated")
    client_parser.add_argument("--top-p", help="Top-p value for text generation")
    client_parser.add_argument("--image", help="Path for image to be analysed")
    ...
    try:
        ...
        if args.command == "server":
            ...
            # Keep the script alive while server runs in the background
            ...
            # Signal handler will attempt cleanup if Ctrl+C was used

        elif args.command == "chat":
            # --- Pass the streaming flag ---
            # If --no-stream is True, streaming should be False.
            ...
            enable_streaming = not args.no_stream
            try:
                send_single_message(message=args.prompt, base_url= f"http://localhost:{args.port}", image= args.image, stream=enable_streaming, temperature= args.temperature, max_tokens = args.max_tokens, top_p = args.top_p)
            except Exception as e:
                console.print("[red]Server process ended unexpectedly.[/red]")
